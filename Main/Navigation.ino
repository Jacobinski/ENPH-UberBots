...
#define SMALLREVERSETIME 4000 //Threshold for straight reverse vs. U turn 
...
int postMiss = 0; //if missed the passenger, set as 1
...
void navigate() {
  while (!fN.isEmpty()) {
    ...
    if (detectCollision()) {
      ...
      int prev = getNode(cN, dir);
      if (millis() - timeLastIntersection > 2000) { // If we are safely past the intersection, turn around normally
        updateParameters(cN_p, prev, dir_p); //Our next node will be where we previously were.
      }
      else { //If we will hit a different intersection -> Always turn left
        cD = dir;
        if (cD == NORTH) {
          dir = WEST;
        }
        ...
        nxt = getNode(prev, dir);
        if (nxt == 999) {  //No connection exists -> 180 deg CCW
          if (cD == NORTH) {
            dir = SOUTH;
          }
          if (cD == SOUTH) {
          ...
          nxt = getNode(prev, dir);
          if (nxt == 999) {  //No connection exists -> 270 deg CCW
            if (cD == NORTH) {
              dir = EAST;
            }
            if (cD == SOUTH) {
        ...
        cN = prev;
        updateParameters(cN_p, nxt, dir_p); //Update the parameters to reflect prev -> nxt
      }
    ...
    if (((left_ir * 5.0 / 1024.0) > IR_THRESH || (right_ir * 5.0 / 1024.0) > IR_THRESH) && passenger == false) {
      ...
      //go forwards for a bit, to avoid IR
      else if (postMiss == 1) {
        int ti = millis(); //Initial time
        int tf = millis(); //Final time
        while (tf - ti < 500) { //Go forward for 1.5 seconds
          followTape();
          tf = millis();
        }
      }
      ...
      if (passenger == true) {
        ...
        //Case 3: Closer to node 4
          ...
          while (!dropoff_path1.isEmpty()) {
            LCD.clear();
            LCD.setCursor(0, 0);
            LCD.print(dropoff_path1.peek());
            fN.push(dropoff_path1.pop());
            delay(500);
          }
          LCD.clear();
          LCD.home();
        ...
        //Case 4: Closer to node 17
          ...
          while (!dropoff_path2.isEmpty()) {
            LCD.clear();
            LCD.setCursor(0, 0);
            LCD.print(dropoff_path2.peek());
            fN.push(dropoff_path2.pop());
            delay(500);
          }
          LCD.clear();
          LCD.home();
    ...
    if (cN == 1 || cN == 7 || cN == 8 || cN == 11 || cN == 15 || cN == 16 || cN == 20) {
      while (!detectCollision()) {
        ...
        if (((left_ir * 5.0 / 1024.0) > IR_THRESH || (right_ir * 5.0 / 1024.0) > IR_THRESH) && passenger == false) {
          ...
          motor.speed(LEFT_MOTOR, 0);
          motor.speed(RIGHT_MOTOR, 0);
          //**************************************************TEST IF A TOKEN CAN BE PASSED, TO NOT BE STUCK ON SAME IR AFTER MISS*********************************************
          if (postMiss == 0) {
            useArm(left_ir, right_ir);
  ...
  void useArm(double left_ir, double right_ir) {
    ...
    if ((left_ir * 5.0 / 1024.0) > IR_THRESH) {
      ...
      for (tryNumber = 1; tryNumber <= 4; tryNumber++) {
        ...
        if (tryNumber == 4 && passenger == false) {
          postMiss = 1;
        }
        //Claw actuation:
        ...
        if (digitalRead(DETECTION_SWITCH) == LOW) {
