  ...
  while(!fN.isEmpty()){
    ...
    if (detectCollision()){
       ...
       if(millis()-timeLastIntersection>2000){ // If we are safely past the intersection, turn around normally
          ...
          char cD = dir; //90 deg CCW
          if (cD == NORTH) {dir = WEST;}
          if (cD == SOUTH) {dir = EAST;}
          if (cD == EAST) {dir = NORTH;}
          if (cD == WEST) {dir = SOUTH;}
          int nxt = getNode(cN,dir);
          ...
          if (nxt == 999) {  //No connection exists -> 180 deg CCW 
             ...
             if (nxt == 999) {  //No connection exists -> 270 deg CCW
               if (cD == NORTH) {dir = EAST;}
               if (cD == SOUTH) {dir = WEST;}
               if (cD == EAST) {dir = SOUTH;}
               if (cD == WEST) {dir = NORTH;}
               int nxt = getNode(cN,dir);
            }
          }
       }
