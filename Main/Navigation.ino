...
void navigate() {
  while (!fN.isEmpty()) {
    ...
    if (paths != 999) { // If there a direction other than forward
      timeLastIntersection = millis();
      if (turnDir == LEFT && (paths == 1 || paths == 3 || paths == 4 || paths == 6)) {
        nav_Intersection();
      }
      else if (turnDir == RIGHT && (paths == 1 || paths == 2 || paths == 4 || paths == 5)) {
        nav_Intersection();
      }
      else if (turnDir == FORWARD && (paths == 1 || paths == 2 || paths == 3)) {
        nav_Intersection();
      }
      else {
        nav_Lost(paths); //Figure out where the hell we are
      }
    }
    ...
    if (((left_ir * 5.0 / 1024.0) > IR_THRESH || (right_ir * 5.0 / 1024.0) > IR_THRESH) && passenger == false) {
      ...
      if ((left_ir * 5.0 / 1024.0) > IR_THRESH) {
        ...
        for (tryNumber = 1; tryNumber < 4; tryNumber++) {
          ...
          if (digitalRead(DETECTION_SWITCH1) == HIGH || digitalRead(DETECTION_SWITCH2) == HIGH) {
...
void nav_Lost(int intsct) {
  ...
  while (found == false) {
    if (intsct != 999 && fourInt == false) {
      ...
      else if (intsct == FR_i || intsct == LR_i || intsct == R_i) {
        LCD.clear();
        LCD.setCursor(0, 0);
        LCD.print("LR FR R");
        delay(2000);
      ...
      else if (intsct == LF_i || intsct == L_i) {
        LCD.clear();
        LCD.setCursor(0, 0);
        LCD.print("LF L");
        delay(2000);
      ...
      if (intsct == LFR_i) {
        LCD.clear();  //We just arrived from reverse
        LCD.setCursor(0, 0);
        LCD.print("LFR I");
        delay(2000);
        turn(RIGHT);
