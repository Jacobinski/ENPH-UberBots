...
volatile unsigned int INT_2 = 0;  //Encoder interupt -> check wheel counts 
volatile unsigned int INT_3 = 0;  //Encoder interupt -> check wheel counts 

void enableExternalInterrupt(unsigned int INTX, unsigned int mode)
{
  if (INTX > 5 || mode > 3 || mode == 1) return; //Return if unreasonable input
  cli();
...
ISR(INT2_vect){
  delay(2);
  if(digitalRead(2) == LOW) {leftWheelCounter++;} //debounce
}

//Right wheel counter
ISR(INT3_vect){
  delay(2);
  if(digitalRead(3) == LOW) {rightWheelCounter++;} //debounce
}